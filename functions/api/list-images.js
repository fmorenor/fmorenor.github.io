export async function onRequest(context) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`R2 LIST error ${response.status}:`, text.substring(0, 500));
      return jsonResponse({ error: `R2 error ${response.status}` }, 500);
    }

    const xml = await response.text();
    const images = parseS3ListResponse(xml, publicDomain);
...
function parseS3ListResponse(xml, publicDomain) {
  const images = [];
